	...
	async (c) => {
		...
		const validatedData = filtered.map((row) => {
			...
			const sanitized = {
				...row,
				Depth_Decline_Proxy:
					row.Depth_Decline_Proxy === null ? null : row.Depth_Decline_Proxy,
				Estimated_SoE_Proxy_Pct:
					row.Estimated_SoE_Proxy_Pct === null
						? null
						: row.Estimated_SoE_Proxy_Pct,
				Estimated_Category:
					row.Estimated_Category === null || row.Estimated_Category === ""
						? null
						: row.Estimated_Category,
				// Ensure booleans are converted from strings if necessary
				Drift_Flag:
					typeof row.Drift_Flag === "boolean"
						? row.Drift_Flag
						: String(row.Drift_Flag).toLowerCase() === "true",
				Alert_Active:
					typeof row.Alert_Active === "boolean"
						? row.Alert_Active
		...
		return c.json(
			ApiSuccessEnvelopeSchema(z.array(HistoricalRowSchema)).parse({
