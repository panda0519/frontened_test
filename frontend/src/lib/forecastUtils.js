export function buildForecastSegment(historicalRows, forecastRows) {
    const result = [];
    // Add historical data - filter out any rows with missing required fields
    const sortedHistorical = [...historicalRows]
        .filter((row) => row.Time !== undefined && row.Water_Level !== undefined)
    ...
    if (sortedHistorical.length > 0 && forecastRows.length > 0) {
        ...
        if (lastHistorical.Water_Level !== undefined &&
            ...
            forecastDate.setMonth(forecastDate.getMonth() + (forecast.Forecast_Horizon_Months || 6));
            result.push({
                date: forecastDate.toISOString().split("T")[0],
                value: lastHistorical.Water_Level +
