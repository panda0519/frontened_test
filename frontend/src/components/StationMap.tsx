...
export function StationMap({
	...
	useEffect(() => {
		if (!mapContainer.current || map.current || !stations) return;
	...
	if (loadError) {
		return (
			<div className="flex flex-col items-center justify-center h-full w-full bg-gray-800 text-red-400 p-4">
	...
	return (
		<div className="relative w-full h-full">
			...
			{warningCount > 0 && (
				<div className="absolute bottom-4 left-4 right-4 md:left-4 md:right-auto md:w-auto z-10">
					<div className="bg-yellow-900/90 border border-yellow-600 text-yellow-100 px-4 py-3 rounded-lg shadow-lg max-w-md">
						<div className="flex items-start gap-2">
							<span className="text-lg">⚠️</span>
							<div>
								<p className="font-medium">Coordinate Warning</p>
								<p className="text-sm text-yellow-300">
									{warningCount} station(s) missing valid coordinates and cannot
									be shown on the map.
								</p>
							</div>
						</div>
